...
def randomized_weighted_procrustes(pts_ref, pts_tar, weights, align_cfg=DEFAULT_ALIGN_CONFIG()):
    ...
    # Initialize VP
    Rt, c = paired_svd(pts_ref_c, pts_tar_c, weights_c)
    c = c.view(batch_size, N, 1).contiguous()
    ...
    c = c[torch.arange(batch_size), best_seed.long()]
    return Rt, c
...
def paired_svd(X, Y, weights: Optional[torch.Tensor] = None):
    ...
    # Calculate scale
    S_fixed = torch.where(R_det[:,  None] < 0, S @ reflect, S) 
    c = 1 /  X.var(-1).sum(-1, keepdim=True) * S_fixed.sum(-1, keepdim=True)

    return Rt.float(), c.float()
